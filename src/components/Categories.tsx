      ...
      <section className="py-3 sm:py-4 md:py-6">
        <div className="container-responsive max-w-7xl mx-auto">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-md p-4 sm:p-6 text-center">
            ...
            <div className="text-red-500 mb-2 sm:mb-3 font-medium text-sm sm:text-base">
              {error}
            </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 sm:mb-4 md:mb-6">
        ...
        <Link
          href="/categories"
          className="text-xs sm:text-sm text-[#6B46C1] hover:text-[#542bc9] font-medium flex items-center group btn-touch whitespace-nowrap"
        >
      ...
      {/* Enhanced responsive category grid with better breakpoints */}
      <div className="grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2 xs:gap-3 sm:gap-4">
        {categories.map((category, index) => (
          <Link
            ...
            href={`/category/${category.title
              .toLowerCase()
              .replace(/\s+/g, "-")}`}
            className="flex flex-col items-center transition-all hover:scale-105 group animate-fade-in"
            style={{ animationDelay: `${index * 50}ms` }}
          >
            {/* Responsive category image container */}
            <div className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 md:w-18 md:h-18 lg:w-20 lg:h-20 relative mb-1.5 xs:mb-2 sm:mb-3 overflow-hidden rounded-full border-2 border-white shadow-lg bg-gradient-to-br from-purple-100 to-indigo-100 group-hover:shadow-xl transition-all duration-300 group-hover:border-[#6B46C1]">
              ...
              category.subCategories[0]?.items[0]?.imageUrl ? (
                <Image
                  ...
                  alt={category.title}
                  fill
                  className="object-cover group-hover:scale-110 transition-transform duration-300 p-0.5 xs:p-1"
                  sizes="(max-width: 475px) 48px, (max-width: 640px) 56px, (max-width: 768px) 64px, (max-width: 1024px) 72px, 80px"
                  onError={(e) => {
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="text-purple-500 text-sm xs:text-base sm:text-lg font-bold">
            ...
            <span className="text-xs xs:text-sm sm:text-sm font-medium text-center text-gray-800 line-clamp-2 max-w-[60px] xs:max-w-[70px] sm:max-w-[80px] md:max-w-[90px] group-hover:text-[#6B46C1] transition-colors leading-tight">
              {category.title}
            </span>
          </Link>
        ))}
